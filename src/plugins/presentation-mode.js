		...
		if ( eventData.settings.presentationMode.use ) {
			...
			window.addEventListener( "message", function( event ) {
				...
				try {
					...
					switch ( json.type ) {
					case "select":
						$.each( eventData.settings.presentationMode.transferredValues,
						function( idx, name ) {
						...
						if ( /[a-z0-9\-]+/i.test( json.targetId ) && typeof json.substep in {
							...
							$.error( "For security reasons the targetId must match " +
									"/[a-z0-9\\-]+/i and substep must be a number." );
						}
						break;
					case "listen":
